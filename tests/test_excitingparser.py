...
def test_gw(parser):
    ...
    assert pytest.approx(sec_sccs[0].section_dos[0].dos_energies[87].magnitude, -1.42127678e-18)
    assert pytest.approx(sec_sccs[0].section_dos[1].dos_energies[-40].magnitude, 1.83109278e-18,)
    assert pytest.approx(sec_sccs[0].section_dos[0].dos_values[0][449], 2.249331609726956e-10)
    assert pytest.approx(sec_sccs[0].section_dos[1].dos_values[0][150], 2.737958376377784e-10)
    ...
    assert pytest.approx(sec_sccs[1].section_k_band[0].section_k_band_segment[1].band_energies[0][2][1].magnitude, -1.13686767e-17)
    assert pytest.approx(sec_sccs[1].section_k_band[1].section_k_band_segment[4].band_energies[0][6][-2].magnitude, 1.50016281e-18)
...
def test_dos_spinpol(parser):
    ...
    assert pytest.approx(sec_doss[0].dos_energies[79].magnitude, -2.98206539e-18)
    assert pytest.approx(sec_doss[0].dos_values[0][126], 1.8925126966797494e-10)
    assert pytest.approx(sec_doss[0].dos_values[1][136], 1.9160612889870427e-11)
    assert pytest.approx(sec_doss[1].dos_energies[240].magnitude, -1.74389789e-19,)
    assert pytest.approx(sec_doss[1].dos_values[0][220], 5.638758214688811e-10)
    assert pytest.approx(sec_doss[1].dos_values[1][78], -4.33359120779702e-10)
